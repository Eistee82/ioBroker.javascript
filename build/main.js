                        ...
                        const req = requestModule.request(url, {
                            ...
                            if (res.statusCode !== 200) {
                                ...
                                res.on('end', () => {
                                    ...
                                    void this.setStateAsync(streamStateId, JSON.stringify({ requestId, content: '', done: true, error: `${detail} (${res.statusCode})` }), true);
                            ...
                            res.on('data', (chunk) => {
                                ...
                                for (const line of lines) {
                                    ...
                                    try {
                                        ...
                                        if (delta) {
                                            fullContent += delta;
                                            void this.setStateAsync(streamStateId, JSON.stringify({ requestId, content: fullContent, done: false }), true);
                                        }
                                    }
                                    catch {
                            ...
                            res.on('end', () => {
                                ...
                                // Send final state
                                void this.setStateAsync(streamStateId, JSON.stringify({ requestId, content: fullContent, done: true }), true);
                                // Cleanup state after delay
                                setTimeout(() => void this.delObjectAsync(streamStateId).catch(() => { }), 30000);
                            });
                        });
                        req.on('error', (err) => {
                            void this.setStateAsync(streamStateId, JSON.stringify({ requestId, content: '', done: true, error: `Connection failed: ${err.message}` }), true);
                            setTimeout(() => void this.delObjectAsync(streamStateId).catch(() => { }), 30000);
                        });
                        req.on('timeout', () => {
                            req.destroy();
                            void this.setStateAsync(streamStateId, JSON.stringify({ requestId, content: '', done: true, error: 'Connection timeout (600s)' }), true);
                            setTimeout(() => void this.delObjectAsync(streamStateId).catch(() => { }), 30000);
                        });
                        req.write(bodyBuffer);
                        req.end();
